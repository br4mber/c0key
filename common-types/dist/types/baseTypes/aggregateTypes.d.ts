...
import { BNString, EncryptedMessage, FactorEnc, ISerializable, IServiceProvider, IStorageLayer, PolyIDAndShares, PolynomialID, ShareDescriptionMap } from "./commonTypes";
...
export interface ITKey extends ITKeyApi, ISerializable {
    ...
    _refreshTSSShares(updateMetadata: boolean, deviceTSSShare: BN, deviceTSSIndex: number, factorPubs: Point[], targetIndexes: number[], vid: string, serverOpts: {
        serverEndpoints: string[];
        serverPubKeys: PointHex[];
        serverThreshold: number;
        selectedServers: number[];
